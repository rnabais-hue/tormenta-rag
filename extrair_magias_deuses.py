# -*- coding: utf-8 -*-
r"""Extração ESTRUTURADA das MAGIAS DIVINAS — *Deuses de Arton* (Cap. 1, págs 62–68).
...
Diferença: filtra o splash de seção "Magias Divinas" (48pt) do nome. `fonte="deuses-arton"`.
Saída: dados/magias_deuses.json. NÃO toca no índice.
"""
...
OUT = BASE / "dados" / "magias_deuses.json"
...
def parse_corpo(sb, sp):
    ...
    rec = {"id": f"magia:deuses:{slug(sb['nome'])}", "tipo": "magia", "nome": sb["nome"],
           "fonte": FONTE, "versao": "1.0", "arcana_divina": sb["tipo"], "circulo": sb["circulo"],
...
def main():
    ...
    banco = {"fonte": FONTE, "livro": "Deuses de Arton", "secao": "Magias Divinas",
    ...
    for m in magias:
        print(f"  {m['nome']:<22} {m['arcana_divina'].capitalize():<9} {m['circulo']} ({m['escola']:<13}) | "
