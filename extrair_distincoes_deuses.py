# -*- coding: utf-8 -*-
r"""Extração ESTRUTURADA das DISTINÇÕES DIVINAS (Cap. 2) de *Deuses de Arton*.

`fonte="deuses-arton"`, `capitulo="distincoes"`, versão Ebook, págs 69–144.
23 Distinções (entidade NOVA). Cada uma: conceito → Admissão (requisito) →
Marca da Distinção (1 poder) → Poderes da distinção (N poderes, cada um com
tag de tipo opcional). PORTE do extractor do Heróis (mesma família de layout):

  - nome da distinção: splash Tormenta20 sz~58 (2 linhas) + drop-cap 78pt
  - seções: Tormenta20 sz18–26  ("Admissão", "Marca da Distinção", "Poderes da distinção")
  - poderes: Tormenta20 sz13.5–18
  - tag do poder (tipo): Tormenta20 sz9.5–13.5 (opcional)
  - corpo/efeito: IowanOldStyle 9–10pt
  - caixas laterais: SourceSansPro → descartadas

DIFERENÇA vs. Heróis: aqui o TOC aponta para a PRÓPRIA página de splash (o início
visual), então os pares (nome, página) do TOC dão os limites diretamente — cada
distinção = da sua página até a página anterior à próxima distinção.
Saída: dados/distincoes_deuses.json. NÃO toca no índice.
"""
...
OUT = Path(__file__).parent / "dados" / "distincoes_deuses.json"
OUT.parent.mkdir(exist_ok=True)
FONTE = "deuses-arton"
VERSAO = "Ebook"
CAP_FIM = 144  # última página do Cap. 2 (Cap. 3 começa na 145)
...
def nomes_toc(doc):
    """23 distinções, em ordem: (nome, página impressa=PDF 1-based). No Deuses o TOC
    aponta para a PÁGINA DE SPLASH (início visual), então a página serve de limite."""
    nomes = []
    for lvl, title, page in doc.get_toc():
        if lvl == 2 and 69 <= page <= CAP_FIM and title.strip() not in (
...
def extrair_distincao(doc, nome, idx_ini, idx_fim, pagina):
    ...
    def junta(lst, dropcap=False):
        s = dehyph(" ".join(lst))
        # cola drop-cap: "P aladinos" -> "Paladinos". SÓ no conceito, cujo início traz o
        # drop-cap 78pt do splash. Marca/poderes começam com corpo normal e podem
        # legitimamente iniciar com "A cada"/"O dragão" — não colar lá.
        if dropcap:
            s = re.sub(r"^([A-ZÀ-Ý]) ([a-zà-ÿ])", r"\1\2", s, count=1)
        return s

    def _cap(s):
        # versalete do nome (16pt) às vezes vem com inicial minúscula
        # ("titereiro Planar" -> "Titereiro Planar")
        return s[0].upper() + s[1:] if s and s[0].islower() else s

    return {
        "id": f"distincao:deuses:{slug(nome)}",
        "tipo": "distincao", "nome": nome, "pagina": pagina,
        "fonte": FONTE, "versao": VERSAO,
        "conceito": junta(conceito, dropcap=True),
        "admissao": junta(admissao),
        "marca": {"nome": _cap(marca["nome"]) if marca["nome"] else None,
                  "efeito": junta(marca["efeito"])},
        "poderes": [{"nome": _cap(p["nome"]), "tag": p["tag"], "subsecao": p["subsecao"],
...
def main():
    ...
    nomes = nomes_toc(doc)
    print(f"nomes TOC={len(nomes)} (esperado 23)")

    # limites: cada distinção vai da sua página de splash até a anterior à próxima
    # (a última fecha no fim do capítulo). Páginas do TOC são 1-based (=PDF idx+1).
    paginas = [p for _, p in nomes]
    dists = []
    for i, (nome, pag) in enumerate(nomes):
        idx_ini = pag - 1
        prox_pag = paginas[i + 1] if i + 1 < len(paginas) else CAP_FIM + 1
        idx_fim = prox_pag - 2
        d = extrair_distincao(doc, nome, idx_ini, idx_fim, pagina=pag)
        dists.append(d)
        npow = len(d["poderes"])
    ...
    dados = {"fonte": FONTE, "livro": "Deuses de Arton", "versao": VERSAO,
